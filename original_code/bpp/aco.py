...
class ACO():
    ...
    def __init__(
        ...
        self.pheromone = torch.ones(len(demand), len(
            demand), device=device) if pheromone is None else pheromone
        self.heuristic = self.demand.unsqueeze(0).repeat(
            len(demand), 1) if heuristic is None else heuristic
        self.heuristic[:, 0] = 1e-5

        self.best_sol = None
        self.best_fitness = torch.tensor(0.0, device=device)
    ...
    def run(self, n_iterations):
        for _ in range(n_iterations):
            ...
            # Local Search
            if self.use_local_search and self.best_sol is not None:
                self.local_search()
        ...
        for i, j in combinations(range(len(edge_sets)), 2):
            jaccard_sum += len(edge_sets[i] & edge_sets[j]) / \
                len(edge_sets[i] | edge_sets[j])
        diversity = 1 - jaccard_sum / \
            (len(edge_sets) * (len(edge_sets) - 1) / 2)

        return self.best_fitness, diversity, self.get_n_bins(self.best_sol)

    def get_n_bins(self, sol):
        if sol is None:
            return 0
        sol = sol.unsqueeze(1)  # (len, 1)
        u = sol.permute(1, 0).cpu().numpy()
        last_zeros = count_last_zero(u)
        n_bins = u.shape[1] - last_zeros - self.problem_size + 1
        return n_bins[0]

    def local_search(self):
        # A simple swap mutation on the best solution
        # This is a placeholder for a more complex local search
        # We try to swap two random non-zero nodes
        current_sol = self.best_sol.clone()

        # Find indices that are not 0 (depot)
        valid_indices = torch.nonzero(current_sol).flatten()
        if len(valid_indices) < 2:
            return

        idx1, idx2 = valid_indices[torch.randperm(len(valid_indices))[:2]]

        # Swap
        current_sol[idx1], current_sol[idx2] = current_sol[idx2], current_sol[idx1]

        # Evaluate
        # We need to reshape to match gen_sol_objs input expectation (n_ants, max_len)
        # But here we have 1 sol.
        # Let's just create a batch of 1
        # (max_len, 1) -> wait gen_sol_objs expects (max_len, n_ants) if passed to it?
        current_sol_batch = current_sol.unsqueeze(1)
        # gen_sol_objs expects (n_ants, max_len)? No, look at gen_sol_objs implementation:
        # u = sols.permute(1, 0).cpu().numpy() -> so sols is (max_len, n_ants)

        # Re-eval
        obj = self.gen_sol_objs(current_sol.unsqueeze(1))[0]

        if obj > self.best_fitness:
    ...
    def update_pheromone(self, sols, fits):
        ...
        if self.elitist:
            ...
            self.pheromone[best_tour[:-1],
                           torch.roll(best_tour, shifts=-1)[:-1]] += best_fit

        else:
            for i in range(self.n_ants):
                sol = sols[:, i]
                fit = fits[i]
                self.pheromone[sol[:-1],
                               torch.roll(sol, shifts=-1)[:-1]] += fit / self.n_ants

        if self.use_mmas:
            self.pheromone = torch.clamp(
                self.pheromone, min=self.tau_min, max=self.tau_max)
        else:
    ...
    def gen_sol_objs(self, sols: torch.Tensor):
        ...
        last_zeros = count_last_zero(u)
        n_bins = u.shape[1] - last_zeros - \
            self.problem_size + 1  # number of bins
        fit = cal_fitness(u, self.demand_numpy, n_bins)
        return torch.tensor(fit, device=self.device, dtype=torch.float)
    ...
    def gen_sol(self, invtemp=1.0, require_prob=False, sols=None):
        ...
        # sols_list[i] is the ith move (tensor) for all ants
        sols_list = [actions]
        ...
        while not done:
            actions, log_probs = self.pick_move(
                actions, visit_mask, capacity_mask, require_prob, invtemp, sols[
                    i + 1] if sols is not None else None
            )
    ...
    def pick_move(self, prev, visit_mask, capacity_mask, require_prob, invtemp=1.0, guiding_node=None):
        ...
        dist = (((pheromone ** self.alpha) * (heuristic ** self.beta))
                ** invtemp) * mask  # shape: (n_ants, p_size)
        # This should be done for numerical stability
        dist = dist / dist.sum(dim=1, keepdim=True)
        dist = Categorical(dist)
        actions = dist.sample() if guiding_node is None else guiding_node  # shape: (n_ants,)
        log_probs = dist.log_prob(
            actions) if require_prob else None  # shape: (n_ants,)
    ...
    def update_capacity_mask(self, cur_nodes, used_capacity):
        ...
        remaining_capacity_repeat = remaining_capacity.unsqueeze(
            -1).repeat(1, self.problem_size)  # (n_ants, p_size)
        demand_repeat = self.demand.unsqueeze(0).repeat(
            self.n_ants, 1)  # (n_ants, p_size)
