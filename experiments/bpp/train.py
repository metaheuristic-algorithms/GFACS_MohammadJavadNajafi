...
def calculate_log_pb_uniform(sols: torch.Tensor):
    ...
    log_p_b_outer = torch.tensor(-scipy.special.gammaln(n_routes + 1),
                                 device=sols.device, dtype=torch.float)

    zero_indices = [np.where(sol == 0)[0] for sol in _sols]
    inner_list = [-scipy.special.gammaln(zero_idx[1:] - zero_idx[:-1]).sum()
                  for zero_idx in zero_indices]
    log_p_b_inner = torch.tensor(
        inner_list, device=sols.device, dtype=torch.float)
...
def train_instance(
    ...
    for _ in range(batch_size):
        ...
        aco = ACO(demands, n_ants=n_ants, heuristic=heu_mat, device=DEVICE,
                  use_mmas=use_mmas, use_local_search=use_local_search)

        objs, log_probs, sols = aco.sample(
            invtemp=invtemp, return_sol=True)  # type: ignore
        ...
        backward_flow = calculate_log_pb_uniform(
            sols.T) + (objs - baseline).detach() * beta

        if loss_type == 'huber':
            tb_loss = torch.nn.functional.huber_loss(
                forward_flow, backward_flow, delta=1.0)
        else:
        ...
        if guided_exploration:
            ...
            with torch.no_grad():
                objs_K, _, sols_K = aco.sample(
                    return_sol=True, K=topk)  # type: ignore

            _, idx = objs_K.topk(topk, largest=True)
            rand_idx = torch.randint(
                0, len(objs_K), (n_ants - topk,), device=DEVICE)
            sols_best = sols_K[:, idx]
            sols_rand = sols_K[:, rand_idx]
            sols_ls = torch.cat([sols_best, sols_rand], dim=1)

            # reset pheromone before calculating log_probs
            aco.pheromone = torch.ones(n_nodes + 1, n_nodes + 1, device=DEVICE)
            ...
            forward_flow_ls = log_probs_ls.sum(
                0) + logZ_ls.expand(n_ants)  # type: ignore
            backward_flow_ls = calculate_log_pb_uniform(
                sols_ls.T) + (objs_ls - baseline_ls).detach() * beta

            if loss_type == 'huber':
                tb_loss_ls = torch.nn.functional.huber_loss(
                    forward_flow_ls, backward_flow_ls, delta=1.0)
            else:
                tb_loss_ls = torch.pow(
                    forward_flow_ls - backward_flow_ls, 2).mean()
    ...
    torch.nn.utils.clip_grad_norm_(
        parameters=model.parameters(), max_norm=3.0, norm_type=2)  # type: ignore
    optimizer.step()
...
def infer_instance(model, instance, n_ants, use_mmas=False, use_local_search=False):
    model.eval()
    demands = instance
    pyg_data = gen_pyg_data(demands, DEVICE)
    n = demands.size(0)
    ...
    # bins not needed here or irrelevant
    best_aco_1, diversity_1, _ = aco.run(1)
    best_aco_T, diversity_T, best_bins = aco.run(T - 1)
    best_aco_1, best_aco_T = best_aco_1.item(), best_aco_T.item()  # type: ignore

    return np.array([baseline, best_sample_obj, best_aco_1, best_aco_T, diversity_1, diversity_T, best_bins])
...
def train_epoch(
    ...
    for i in tqdm(range(steps_per_epoch), desc="Train", dynamic_ncols=True):
        ...
        train_instance(
            net, optimizer, batch_size, n_nodes, n_ants, invtemp, guided_exploration, shared_energy_norm, beta, topk, it, loss_type, use_mmas, use_local_search
        )


@torch.no_grad()
def validation(val_list, n_ants, net, epoch, steps_per_epoch, use_mmas=False, use_local_search=False):
    stats = []
    for _, instance in tqdm(enumerate(val_list)):
        stats.append(infer_instance(
            net, instance, n_ants, use_mmas, use_local_search))
...
def train(
    ...
    run_name="",
    # (invtemp_min, invtemp_max, invtemp_flat_epochs)
    invtemp_schedule_params=(0.8, 1.0, 5),
    guided_exploration=False,
    shared_energy_norm=False,
    ...
    net = Net(gfn=True, Z_out_dim=2 if guided_exploration else 1,
              model_type=model_type).to(DEVICE)
    if pretrained is not None:
        raise NotImplementedError(
            "Need some more work to load pretrained model")
    ...
    best_result = validation(val_list, n_val_ants, net,
                             0, steps_per_epoch, use_mmas, use_local_search)

    sum_time = 0
    for epoch in range(1, epochs + 1):
        # Heatmap Inverse Temperature Schedule
        invtemp_min, invtemp_max, invtemp_flat_epochs = invtemp_schedule_params
        if epochs - invtemp_flat_epochs <= 0:
            invtemp = invtemp_max
        else:
            invtemp = invtemp_min + (invtemp_max - invtemp_min) * \
                min((epoch - 1) / (epochs - invtemp_flat_epochs), 1.0)

        # Beta Schedule
        beta_min, beta_max, beta_flat_epochs = beta_schedule_params
        if epochs - beta_flat_epochs <= 1:
            beta = beta_max
        else:
            beta = beta_min + (beta_max - beta_min) * min(math.log(epoch) /
                                                          math.log(epochs - beta_flat_epochs), 1.0)
        ...
        if epoch % val_interval == 0:
            curr_result = validation(
                val_list, n_val_ants, net, epoch, steps_per_epoch, use_mmas, use_local_search)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-l", "--lr", metavar='η', type=float,
    ...
    parser.add_argument("-a", "--ants", type=int, default=30,
                        help="Number of ants (in ACO algorithm)")
    parser.add_argument("-va", "--val_ants", type=int,
                        default=100, help="Number of ants for validation")
    parser.add_argument("-b", "--batch_size", type=int,
                        default=5, help="Batch size")
    parser.add_argument("-s", "--steps", type=int,
                        default=20, help="Steps per epoch")
    parser.add_argument("-e", "--epochs", type=int,
                        default=20, help="Epochs to run")
    parser.add_argument("-t", "--val_size", type=int,
                        default=10, help="Number of instances for validation")
    parser.add_argument("-o", "--output", type=str, default="../pretrained/bpp",
                        help="The directory to store checkpoints")
    parser.add_argument("--val_interval", type=int, default=5,
                        help="The interval to validate model")
    # Logging
    ...
    parser.add_argument("--invtemp_min", type=float, default=1.0,
                        help='Inverse temperature min for GFACS')
    parser.add_argument("--invtemp_max", type=float, default=1.0,
                        help='Inverse temperature max for GFACS')
    parser.add_argument("--invtemp_flat_epochs", type=int, default=5,
                        help='Inverse temperature glat rpochs for GFACS')
    # GFACS
    parser.add_argument("--beta_min", type=float,
                        default=None, help='Beta min for GFACS')
    parser.add_argument("--beta_max", type=float,
                        default=None, help='Beta max for GFACS')
    parser.add_argument("--beta_flat_epochs", type=int,
                        default=5, help='Beta flat epochs for GFACS')
    parser.add_argument("--disable_shared_energy_norm", action='store_true',
                        help='Disable shared energy normalization for GFACS')
    # Top-k guided exploration
    parser.add_argument("--disable_guided_exp", action='store_true',
                        help='Disable guided exploration for GFACS')
    parser.add_argument("--topk", type=int, default=5,
                        help="TopK for guided exploration")
    # Seed
    parser.add_argument("--seed", type=int, default=0, help="Random seed")
    # Experiments
    parser.add_argument("--model_type", type=str, default="embnet",
                        choices=["embnet", "gcn"], help="Model architecture")
    parser.add_argument("--loss_type", type=str, default="mse",
                        choices=["mse", "huber"], help="Loss function")
    parser.add_argument("--use_mmas", action="store_true",
                        help="Use Max-Min Ant System")
    parser.add_argument("--use_local_search",
                        action="store_true", help="Use Local Search")
    ...
    pretrained_name = (
        args.pretrained.replace("../pretrained/bpp/",
                                "").replace("/", "_").replace(".pt", "")
    ...
    train(
        ...
        invtemp_schedule_params=(
            args.invtemp_min, args.invtemp_max, args.invtemp_flat_epochs),
        guided_exploration=(not args.disable_guided_exp),
        shared_energy_norm=(not args.disable_shared_energy_norm),
        beta_schedule_params=(args.beta_min, args.beta_max,
                              args.beta_flat_epochs),
        topk=args.topk,
